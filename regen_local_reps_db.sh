...
rm -f dump.sql
./convert_reps_json.pl --dump dump.sql

#LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 sqlite3 reps.db ".dump" | grep '^INSERT INTO' | sed -E 's/unistr\(([^)]*)\)/\1/g'  > dump.sql
